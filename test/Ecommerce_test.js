...
contract("Ecommerce", (accounts) => {
  ...
  it("debería fallar si el comprador envía fondos insuficientes", async () => {
    ...
    try {
      ...
      assert(error.message.includes("insuficient"), "Se esperaba un error por fondos insuficientes");
    }
  });

  //Test Producto vendido - No comprar
  it("no debería permitir comprar un producto ya vendido", async () => {
    ...
    try {
      ...
      assert(error.message.includes("vendido"), "Se esperaba un error por producto ya vendido");
    }
  });
});
